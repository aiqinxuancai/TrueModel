...
- **糖果检测**：每轮检测额外请求一次固定糖果题，支持单个模型或全部模型单独刷新，在两张模型表、最近检测浮层和详情中展示结果、题目与回复。题目与判分规则参考 [haowang02/codex-candy-eval](https://github.com/haowang02/codex-candy-eval/blob/29127fa5a12fb7654e865f684dcaf55ade181349/codex_candy_eval.py)：回复中出现前后非数字的 `21` 即通过，否则未通过；接口异常显示错误，不影响归因结果。沿用上游的文本匹配判分，不分析推理过程是否正确。通过已配置的模型 API 调用，无需本地 Codex CLI；不复现上游 CLI 的上下文、reasoning effort 或 token 统计。替换原“是/否检测”，旧记录显示 `—`，重新检测后生成糖果结果。
