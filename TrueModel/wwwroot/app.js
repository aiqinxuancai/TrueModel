...
const query=$('search').value.toLowerCase();$('modelRows').innerHTML=models.filter(x=>[x.s.name,x.k.name,x.m.name].join(' ').toLowerCase().includes(query)).map(({s,k,m,r})=>{const winner=r?.attributionJson?JSON.parse(r.attributionJson).Candidates[0]:null;return `<tr><td>${esc(s.name)}</td><td>${esc(k.name)}</td><td>${esc(m.name)}</td><td>${badge(modelStatus(m.id,r))}</td><td>${r?(r.latencyMs/1000).toFixed(1)+'s':'—'}</td><td>${attributionLabel(winner,m.name)}</td><td title="Juice 为模型自报值，仅供参考">${juiceLabel(r)}</td><td>${winner?(winner.Probability*100).toFixed(1)+'%':'—'}</td><td class="recent-model-cell">${recentModelHistory(m.id)}</td><td><button data-detect-model="${m.id}">检测</button>${r?` <button data-detail="${r.id}">详情</button>`:''}</td></tr>`;}).join('')||'<tr><td colspan="10" class="empty">暂无模型</td></tr>';
