...
function recentResultsForModel(modelId) {
    return recentModelResults.filter(r => r.modelId === modelId).slice(0, 10);
}
function attributionWinner(r) {
    try { return JSON.parse(r?.attributionJson || 'null')?.Candidates?.[0]; } catch { return null; }
}
function attributionResultType(r) {
    const winner = attributionWinner(r);
    const normalize = value => String(value || '').trim().toLowerCase();
    return r.status !== 'Success' ? 'failure' : !winner ? 'unknown' : normalize(winner.Model || winner.DisplayName) === normalize(r.modelName) ? 'success' : 'mismatch';
}
function recentAttributionAccuracy(modelId) {
    const recent = recentResultsForModel(modelId);
    return recent.length ? recent.filter(r => attributionResultType(r) === 'success').length / recent.length : 0;
}
...
async function refresh(){[sites,results,runs,banks,recentModelResults]=await Promise.all(['/sites','/results','/runs','/banks','/results?perModel=true'].map(p=>api(p)));render();const settings=await api('/settings');if(!document.querySelector('#settingsForm:focus-within'))for(const name of ['challengeCount','intervalMinutes','maxConcurrency','timeoutSeconds','candyReasoningEffort'])$('settingsForm').elements[name].value=name==='timeoutSeconds'?240:settings[name];$('nextRun').textContent=`每次 ${settings.challengeCount} 个挑战 · 下次定时检测：${time(settings.nextRunAt)}`;}
...
const accurateModels=models.filter(x=>recentAttributionAccuracy(x.m.id)>=0.8);
$('accurateModelCount').textContent=accurateModels.length;
$('accurateModelRows').innerHTML=accurateModels.map(modelRow).join('')||'<tr><td colspan="10" class="empty">暂无最近归因正确率 ≥80% 的模型</td></tr>';
...
$('bankRows').innerHTML=banks.map(b=>`<div class="row"><div><b>${esc(b.name)}</b> ${b.active?badge('Success'):''}<small>${time(b.importedAt)} · SHA256 ${esc(b.sha256)}</small></div><div class="actions"><button data-bank-detail="${b.id}">查看</button><button data-manual-bank="${b.id}">手动新增</button><a href="/api/banks/${b.id}/export">导出</a>${!b.active?`<button data-activate="${b.id}">设为当前库</button><button data-delete-bank="${b.id}">删除</button>`:''}</div></div>`).join('')||'<p class="empty">暂无指纹库</p>';}
...
$('createEmptyBank').onclick=()=>edit('新建空指纹库',[['name','指纹库名称']],async v=>{await api('/banks/empty','POST',{name:v.name});message('空指纹库已创建，可手动新增或采集指纹；至少两个模型后可设为当前库');});
